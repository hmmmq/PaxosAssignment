...
public class MemberRunnable implements Runnable {
            ...
            synchronized (failure) {
                ...
                if (failure.get() ==  true) {
                    if (timeToRestart > 0) {
                        ...
                        System.out.println(String.format("Member %d is now unavailable", id+1));
                    }
                // If we're in state of liveness, set a timer for failure
                    ...
                    failure.notifyAll();
                    timer.setTimeout(Thread.currentThread(), timeToFail);
                    System.out.println(String.format("Member %d is now back", id+1));
                }
            }
        }
    ...
    private void gracefulShutdown() {
        ...
        System.out.println(String.format("Member %d Shutting down", id+1));
    }
}
