...
public class AcceptorRunnable implements Runnable {
    ...
    private void handlePrepare(String message) {
        ...
        if (n > maxPromise) {
            ...
            System.out.println(String.format("Member %d promises proposal %d", id+1, n));
            maxPromise = n;
        } else {
            message = String.format("%c%d %d", PREPARENACK, id, n);
            System.out.println(String.format("Member %d cannot promise proposal %d", id+1, n));
        }
    ...
    private void handleProposal(String message) {
        ...
        if (maxPromise <= n) {
            ...
            System.out.println(String.format("Member %d accepts proposal %d with value %d", id+1, n, v));

        //Send a PROPOSALNACK if we did promise
        } else {
            sender.send(String.format("%c%d %d", PROPOSALNACK, id, n), from);
            System.out.println(String.format("Member %d cannot accept proposal %d", id+1, n));
        }
    }
}
